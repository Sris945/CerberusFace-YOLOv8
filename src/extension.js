...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('ai-project-refactorer.start', () => {
        const panel = vscode.window.createWebviewPanel('aiProjectRefactorer', // viewType: unique identifier
...
async function setupPythonEnvironment(context, progress) {
    ...
    const pythonCommand = config.get('pythonPath') || 'python';
    ...
    const runCommand = (cmd, args) => new Promise((resolve, reject) => {
        const process = (0, child_process_1.spawn)(cmd, args, { cwd: agentPath });
...
async function runAgent(context, projectPath, formData, progress, token // Add the cancellation token parameter
) {
    try {
        ...
        const config = vscode.workspace.getConfiguration('ai-project-refactorer');
        const ibmApiKey = config.get('ibmApiKey');
        const ibmUrl = config.get('ibmUrl') || 'https://us-south.ml.cloud.ibm.com';
        const ibmProjectId = config.get('ibmProjectId');
        ...
        const agentProcess = (0, child_process_1.spawn)(pythonPath, args, {
            env: {
                ...process.env,
